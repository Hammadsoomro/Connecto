...
export const sendSMS: RequestHandler = async (req, res) => {
  try {
    ...
    // If this is a sub-account, verify they can only use their assigned number
    if (req.subAccount) {
      if (!req.subAccount.assignedNumber) {
        return res.status(403).json({ error: "No phone number assigned to this sub-account" });
      }
      if (fromNumber !== req.subAccount.assignedNumber) {
        return res.status(403).json({ error: "Sub-account can only send from assigned number" });
      }
    }
