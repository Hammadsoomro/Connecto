...
export default function ContactList({
  ...
  return (
    <div
      className={`h-full flex flex-col ${theme === "dark" ? "bg-black/90" : "bg-black/20"} backdrop-blur-sm border-r ${theme === "dark" ? "border-white/10" : "border-white/10"}`}
    >
      {/* Header */}
      <div
        className={`p-4 border-b ${theme === "dark" ? "border-white/10" : "border-white/10"}`}
      >
            ...
            <Dialog open={isAddContactOpen} onOpenChange={setIsAddContactOpen}>
              <DialogTrigger asChild>
                <Button
                  size="sm"
                  variant="outline"
                  className={`bg-transparent ${theme === "dark" ? "border-white/20 text-white hover:bg-white/10" : "border-white/30 text-white hover:bg-white/10"}`}
                >
        ...
        <div className="relative">
          ...
          <Input
            ...
            className={`pl-10 ${theme === "dark" ? "bg-gray-800 border-gray-600 text-white placeholder:text-gray-400" : "bg-white/10 border-white/20 text-white placeholder:text-gray-300"}`}
          />
        </div>
      </div>
    ...
    <div
      className={cn(
        ...
        contact.unreadCount > 0 &&
          !isSelected &&
          (theme === "dark" ? "bg-purple-950/30" : "bg-white/10"),
      )}
      onClick={onSelect}
    >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            ...
            {contact.lastMessageAt && (
              <div className={`text-xs ${theme === "dark" ? "text-gray-400" : "text-gray-300"}`}>
                {formatTime(new Date(contact.lastMessageAt))}
              </div>
            )}
          </div>

          <div className={`text-sm truncate ${theme === "dark" ? "text-gray-300" : "text-gray-300"}`}>
            {contact.lastMessage ? (
              <div className="flex items-center gap-1">
                ...
                <span
                  className={cn(
                    "truncate",
                    contact.unreadCount > 0 && (theme === "dark" ? "font-medium text-white" : "font-medium text-white"),
                  )}
                >
